...
#call as: bash callTmOnFullNatives.sh /home/matt/project/data/split_doms/ /home/matt/project/data/CASP10_full_structures_official/ /home/matt/project/data/rawData/round1output/splitGDTscores/splitAgainstOfficialFull/
            ...
            if [[ $multiDomains =~ $filename ]]; then 
                ...
                for path in $splitdoms/*; do
                    ...
                    if [[ $GDTline =~ GDT-TS-score=[[:space:]]+([0-9].[0-9]{4})[[:space:]] ]]; then
                        ...
                        #echo $score
                        if [[ $i -eq 0 ]]; then
                            echo "Target,Domain,Model,GDTscore" >>"$3GDTscoresForFullNativesSplitModels.csv"
                        fi
                        let i++
                        line="$filename,$domain,$modelname,$score"
                        echo "$line" >>"$3GDTscoresForFullNativesSplitModels.csv"
                    fi
                    score=""
                    GDTline=""
                done
